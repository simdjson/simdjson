...
int main() {
    ...
    auto error = parser.iterate_many(view, 65536, simdjson::stream_format::whitespace_delimited).get(stream);
#else
    simdjson::padded_string json(input);
    auto error = parser.iterate_many(json, 65536, simdjson::stream_format::whitespace_delimited).get(stream);
#endif
    if (error) {
