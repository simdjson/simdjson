...
namespace scalar_tests {
  ...
  template<typename T>
  bool test_scalar_value(const std::string &json, const T &expected, bool test_twice=true) {
    ...
    {
      std::string whitespace_json = std::string(json) + " ";
    ...
    {
      std::string array_json = std::string("[") + std::string(json) + "]";
    ...
    {
      std::string whitespace_array_json = std::string("[") + std::string(json) + " ]";
  ...
  bool string_value() {
    ...
    if (!test_scalar_value(R"("hi")", std::string_view("hi"), false)) { return false; }
    // ... unless the document is big enough to have a big string buffer :)
    if (!test_scalar_value(R"("hi"        )", std::string_view("hi"))) { return false; }
    TEST_SUCCEED();
  }

  bool numeric_values() {
    TEST_START();
    if (!test_scalar_value<int64_t> ("0",   0)) { return false; }
    if (!test_scalar_value<uint64_t>("0",   0)) { return false; }
    if (!test_scalar_value<double>  ("0",   0)) { return false; }
    if (!test_scalar_value<int64_t> ("1",   1)) { return false; }
    if (!test_scalar_value<uint64_t>("1",   1)) { return false; }
    if (!test_scalar_value<double>  ("1",   1)) { return false; }
    if (!test_scalar_value<int64_t> ("-1",  -1)) { return false; }
    if (!test_scalar_value<double>  ("-1",  -1)) { return false; }
    if (!test_scalar_value<double>  ("1.1", 1.1)) { return false; }
    TEST_SUCCEED();
  }

  bool boolean_values() {
    TEST_START();
    if (!test_scalar_value<bool> ("true",  true)) { return false; }
    if (!test_scalar_value<bool> ("false", false)) { return false; }
    TEST_SUCCEED();
  }
  ...
  template<typename T>
  bool test_scalar_value_exception(const std::string &json, const T &expected) {
    std::cout << "- JSON: " << json << endl;
    SUBTEST( "document", test_ondemand_doc(json, [&](auto doc_result) {
      ondemand::document doc;
      ASSERT_SUCCESS( std::move(doc_result).get(doc) );
  ...
  bool string_value_exception() {
    TEST_START();
    return test_scalar_value_exception(R"("hi")", std::string_view("hi"));
  }

  bool numeric_values_exception() {
    TEST_START();
    if (!test_scalar_value_exception<int64_t> ("0",   0)) { return false; }
    if (!test_scalar_value_exception<uint64_t>("0",   0)) { return false; }
    if (!test_scalar_value_exception<double>  ("0",   0)) { return false; }
    if (!test_scalar_value_exception<int64_t> ("1",   1)) { return false; }
    if (!test_scalar_value_exception<uint64_t>("1",   1)) { return false; }
    if (!test_scalar_value_exception<double>  ("1",   1)) { return false; }
    if (!test_scalar_value_exception<int64_t> ("-1",  -1)) { return false; }
    if (!test_scalar_value_exception<double>  ("-1",  -1)) { return false; }
    if (!test_scalar_value_exception<double>  ("1.1", 1.1)) { return false; }
    TEST_SUCCEED();
  }

  bool boolean_values_exception() {
    TEST_START();
    if (!test_scalar_value_exception<bool> ("true",  true)) { return false; }
    if (!test_scalar_value_exception<bool> ("false", false)) { return false; }
    TEST_SUCCEED();
  }
