...
namespace object_tests {
  ...
  bool missing_key_continue() {
    TEST_START();
    simdjson::ondemand::parser parser;
    std::string_view docdata =  R"({"a":0, "b":1, "c":2})";
    simdjson::ondemand::document doc;
  ...
  bool broken_issue_1521() {
    ...
    std::string_view json = R"({"type":"root","nodes":[{"type":"child","nodes":[]},{"type":"child","name":"child-name","nodes":[]}]})";
  ...
  bool fixed_broken_issue_1521() {
    ...
    ondemand::parser parser;
    // We omit the ',"nodes":[]'
    std::string_view json = R"({"type":"root","nodes":[{"type":"child"},{"type":"child","name":"child-name","nodes":[]}]})";
    ondemand::document file_tree = parser.iterate(json);
    try {
      broken_descend(file_tree);
    } catch(simdjson::simdjson_error& e) {
  ...
  bool iterate_object() {
    TEST_START();
    auto json = std::string_view(R"({ "a": 1, "b": 2, "c": 3 })");
  ...
  bool value_search_unescaped_key() {
    TEST_START();
    auto json = std::string_view(R"({"k\u0065y": 1})");
  ...
  bool issue_1480() {
    TEST_START();
    auto json = std::string_view(R"({ "name"  : "something", "version": "0.13.2", "version_major": 0})");
