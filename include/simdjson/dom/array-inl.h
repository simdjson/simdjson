...
simdjson_inline
simdjson_result<dom::array>::simdjson_result(dom::array value) noexcept
    : internal::simdjson_result_base<dom::array>(
          std::forward<dom::array>(value)) {}
simdjson_inline
simdjson_result<dom::array>::simdjson_result(error_code error) noexcept
    : internal::simdjson_result_base<dom::array>(error) {}

#if SIMDJSON_EXCEPTIONS

inline dom::array::iterator simdjson_result<dom::array>::begin() const
    noexcept(false) {
  if (error()) {
...
inline dom::array::iterator simdjson_result<dom::array>::end() const
    noexcept(false) {
  if (error()) {
    throw simdjson_error(error());
  }
...
inline simdjson_result<dom::element> simdjson_result<dom::array>::at_pointer(
...
inline simdjson_result<dom::element> simdjson_result<dom::array>::at_path(
...
simdjson_result<dom::array>::at(size_t index) const noexcept {
  if (error()) {
    return error();
  }
...
simdjson_inline array::array(const internal::tape_ref &_tape) noexcept
    : tape{_tape} {}
...
array::at_pointer(std::string_view json_pointer) const noexcept {
  SIMDJSON_DEVELOPMENT_ASSERT(
      tape.usable()); // https://github.com/simdjson/simdjson/issues/1914
  if (json_pointer
  ...
  // We don't support this, because we're returning a real element, not a
  // position.
  if (json_pointer == "-") {
  ...
  for (i = 0; i < json_pointer.length() && json_pointer[i] != '/'; i++) {
    ...
    // Check for non-digit in array index. If it's there, we're trying to get a
    // field in an object
    if (digit > 9) {
...
inline simdjson_result<element>
array::at_path(std::string_view json_path) const noexcept {
  auto json_pointer = json_path_to_pointer_conversion(json_path);
  if (json_pointer == "-1") {
    return INVALID_JSON_POINTER;
  }
...
//
simdjson_inline
array::iterator::iterator(const internal::tape_ref &_tape) noexcept
    : tape{_tape} {}
...
inline bool
array::iterator::operator!=(const array::iterator &other) const noexcept {
...
#if SIMDJSON_EXCEPTIONS
static_assert(
    std::ranges::view<simdjson::simdjson_result<simdjson::dom::array>>);
static_assert(
    std::ranges::sized_range<simdjson::simdjson_result<simdjson::dom::array>>);
#endif // SIMDJSON_EXCEPTIONS
