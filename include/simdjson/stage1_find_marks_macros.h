...
#ifdef ARCHITECTURE
template <>
really_inline uint64_t find_odd_backslash_sequences<ARCHITECTURE>(simd_input<ARCHITECTURE> in, uint64_t &prev_iter_ends_odd_backslash) {
    const uint64_t even_bits = 0x5555555555555555ULL;                        
    const uint64_t odd_bits = ~even_bits;                                    
    uint64_t bs_bits = cmp_mask_against_input<ARCHITECTURE>(in, '\\');                  
...
template <>
really_inline uint64_t find_quote_mask_and_bits<ARCHITECTURE>(
    simd_input<ARCHITECTURE> in, uint64_t odd_ends,
    uint64_t &prev_iter_inside_quote, uint64_t &quote_bits,
    uint64_t &error_mask) {                                                              
    quote_bits = cmp_mask_against_input<ARCHITECTURE>(in, '"');                         
    quote_bits = quote_bits & ~odd_ends;                                     
    uint64_t quote_mask = compute_quote_mask<ARCHITECTURE>(quote_bits);                 
    ...
    uint64_t unescaped = unsigned_lteq_against_input<ARCHITECTURE>(in, 0x1F);           
...
void find_structural_bits_64(const uint8_t *buf,
                              size_t idx,
                              uint32_t *base_ptr,
                              uint32_t &base,
                              ...
                              uint64_t &error_mask,
                              utf8_checking_state<ARCHITECTURE> &utf8_state)                                            
  {                                                                          
    simd_input<ARCHITECTURE> in = fill_input<ARCHITECTURE>(buf);                                   
    check_utf8<ARCHITECTURE>(in, utf8_state);                                           
    /* detect odd sequences of backslashes */                                
    uint64_t odd_ends =                                                      
        find_odd_backslash_sequences<ARCHITECTURE>(in, prev_iter_ends_odd_backslash);   
                                                                             
    /* detect insides of quote pairs ("quote_mask") and also our quote_bits  
     * themselves */                                                         
    uint64_t quote_bits;                                                     
    uint64_t quote_mask = find_quote_mask_and_bits<ARCHITECTURE>(                       
     ...
     * and flatten */
#ifdef IS_X86_64
    if (ARCHITECTURE == Architecture::HASWELL)
    ...
    uint64_t whitespace;                                                     
    find_whitespace_and_structurals<ARCHITECTURE>(in, whitespace, structurals);         
...
template <>
int find_structural_bits<ARCHITECTURE>(const uint8_t *buf, size_t len,
                                                ParsedJson &pj)
  {                                                                          
    ...
    for (; idx < lenminus64; idx += 64) {                                    
      find_structural_bits_64(                                               
          &buf[idx], idx, base_ptr, base, prev_iter_ends_odd_backslash,   
    ...
    if (idx < len) {                                                         
      ...
      find_structural_bits_64(                                               
          &tmp_buf[0], idx, base_ptr, base, prev_iter_ends_odd_backslash, 
          prev_iter_inside_quote, prev_iter_ends_pseudo_pred, structurals,   
          error_mask, utf8_state);                                     
      idx += 64;                                                             
    }                                                                        
    ...
    return check_utf8_errors<ARCHITECTURE>(utf8_state);                                 
  }
