...
namespace simdjson {
template <typename T>
constexpr bool require_custom_deserialization = false;
...
template <std::unsigned_integral T>
  requires(!require_custom_deserialization<T>)
...
!concepts::appendable_containers<T> && !require_custom_deserialization<T>);
