...
consteval uint64_t emulu(uint32_t x, uint32_t y) {
  return x * (uint64_t)y;
}
...
consteval bool compute_float_64(int64_t power, uint64_t i, bool negative, double &d) {
  if(i == 0) {
    d = negative ? -0.0 : 0.0;
    return true;
  }
...
consteval std::pair<double,size_t> parse_double(const char * src, const char * end) {
  ...
  while (parse_digit(get_value(p), i)) { p++; }
  if ( p == src ) { simdjson_consteval_error("Invalid float value"); }
  if ( (leading_zero && p != src+1)) { simdjson_consteval_error("Invalid float value"); }
  int64_t exponent = 0;
  bool overflow;
  if (get_value(p) == '.') {
    p++;
    const char *start_decimal_digits = p;
    if (!parse_digit(get_value(p), i)) { simdjson_consteval_error("Invalid float value"); } // no decimal digits
    p++;
    while (parse_digit(get_value(p), i)) { p++; }
    ...
    if (overflow && leading_zero) {
      ...
      while (get_value(start_digits) == '0') { start_digits++; }
      overflow = p-start_digits > 19;
    }
  } else {
    overflow = p-src > 19;
  }
  if (overflow) { simdjson_consteval_error("Overflow while computing the float value: too many digits"); }
  if (get_value(p) == 'e' || get_value(p) == 'E') {
    ...
    while (parse_digit(get_value(p), exp)) { p++; }
    if (p-start_exp_digits == 0 || p-start_exp_digits > 19) { simdjson_consteval_error("Invalid float value"); }
    exponent += exp_neg ? 0-exp : exp;
  }

  overflow = overflow || exponent < simdjson::internal::smallest_power || exponent > simdjson::internal::largest_power;
  if (overflow) { simdjson_consteval_error("Overflow while computing the float value"); }
  double d;
  if (!compute_float_64(exponent, i, negative, d)) { simdjson_consteval_error("Overflow while computing the float value"); }
...
parse_string(std::string_view json) {
  ...
  auto process_escaped_unicode = [&] [[nodiscard]] () -> bool {
    ...
    auto codepoint_to_utf8 = [&out][[nodiscard]](uint32_t cp) -> bool {
      //  the high and low surrogates U+D800 through U+DFFF are invalid code points
      ...
      if (how_many_backslashes % 2 == 1) {
        ...
        case 'u':
          if (!process_escaped_unicode()) {
            simdjson_consteval_error("Invalid unicode escape sequence in string");
          }
          break;
        default:
...
parse_json_array_impl(std::string_view &json) {
  ...
  if (!expect_consume('[')) {
    simdjson_consteval_error("Expected '[', but found");
  }
...
parse_json_object_impl(std::string_view json) {
  ...
  // 1. Create the type: class_type<member_specs...>
